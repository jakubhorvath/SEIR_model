...
#TODO check in the article that this is correct
ASYMP_RECOVERY = 1/1.367
MILD_RECOVERY = 1/1.367
...
def R0():
    ...
    for i in range(10):
        if not LOCKDOWN_MEASURES:
            q = sum(SOCIAL_CONTACT_MATRIX_ASYMP[i]) / (sum(CITIZENS_SLOVAKIA))
        else:
            q = sum(SOCIAL_CONTACT_MATRIX_INTERVENTION[i]) / (sum(CITIZENS_SLOVAKIA) )
...
def FOI(asymptomatic_infected, symptomatic_infected):    # λ
    ...
    if not LOCKDOWN_MEASURES:
        infection_rate = SOCIAL_CONTACT_MATRIX_ASYMP / (sum(CITIZENS_SLOVAKIA))
    else:
        infection_rate = SOCIAL_CONTACT_MATRIX_INTERVENTION / (sum(CITIZENS_SLOVAKIA))
    infection_rate_symp = infection_rate * SYMP_INFECTIOUS_FACTOR
    infection_rate_asymp = infection_rate * ASYMP_INFECTIOUS_FACTOR
